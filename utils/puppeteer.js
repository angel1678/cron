...
const dateFormat = (fecha) => {
  const value = fecha
    ? dayjs(fecha, "DD/MM/YYYY HH:mm")
    : dayjs().tz("America/Guayaquil");
...
const procesarDetalle = async (procesos = []) => {
  ...
  for (const {
    ...
    try {
      ...
      const causasTotal = (
        await page.$$("section[class='causas'] > div[class='cuerpo'] > div")
      ).length;

      for (let h = 0; h < causasTotal; h++) {
        await page.waitForTimeout(2000);
        let causa = (
          await page.$$("section[class='causas'] > div[class='cuerpo'] > div")
        )[h];
        await causa.$eval("div > a", (e) => e.click());
        await page.waitForSelector(
          "div[class='lista-movimientos-causa'] div[class='movimiento-individual ng-star-inserted']"
        );
        ...
        for (let i = 0; i < movimientosCount; i++) {
          ...
          if (movimiento) {
            const numeroIncidente = await movimiento.$eval(
              "div[class='numero-incidente']",
              (e) => e.innerHTML.trim()
            );
            const fechaIngreso = await movimiento.$eval(
            ...
            await page.waitForSelector(
              "section[class='panel-expansion'] > mat-accordion > mat-expansion-panel"
            );

            const movimientoInfo = (
              await page.$$(
                "header[class='cabecera-informacion'] section[class='filtros-busqueda']"
              )
            )[1];

            const movimientoDato = await movimientoInfo.$$eval(
              "div > span",
              (e) => e.map((item) => item.innerHTML.trim())
            );
            ...
            const movimientoDB = await select(
              ...
              [proceso_id, movimiento_id]
            );

            if (movimientoDB.length == 0) {
              const createAt = dateFormat();

              await insert("procesos_movimiento", {
                id: movimiento_id,
                proceso_id,
            ...
            const detalleMovimiento = await page.$$(
              "mat-accordion[id='actuaciones-judiciales'] > mat-expansion-panel"
            );
            for (const detalle of detalleMovimiento) {
              const headerDato = await detalle.$$eval(
                "mat-expansion-panel-header div > span",
                (e) => e.map((item) => item.innerHTML.trim())
              );

              const bodyDato = await detalle.$eval(
                "article[class='actividad pagina']",
                (e) => e.textContent.trim()
              );

              const fecha = dateFormat(headerDato[0]);
              const detalle_id = getUUID(
                `${movimiento_id}-${fecha}-${headerDato[1]}-${bodyDato}`
              );
              const createAt = dateFormat();

              const data = await select(
                `SELECT * FROM procesos_detalle WHERE movimiento_id = ? AND id = ?`,
                [movimiento_id, detalle_id]
              );
              if (data.length == 0) {
                await insert("procesos_detalle", {
                  id: detalle_id,
                  movimiento_id,
                  fecha,
                  titulo: headerDato[1],
        ...
        const [detalle] = await select(
          `SELECT * FROM procesos_detalle WHERE movimiento_id = ? ORDER BY fecha DESC limit 1`,
          [movimiento_id]
        );
      ...
      await update(
        "procesos",
        { accion_infraccion: accionInfraccion, executed_at: dateFormat() },
        { id: proceso_id }
      );
    } catch (e) {
